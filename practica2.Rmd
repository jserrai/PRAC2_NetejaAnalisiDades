...
### 3.2.  Comprovació de la normalitat i homogeneïtat de la variància.
```{r echo=TRUE, message=FALSE, warning=FALSE, fig.width=10, fig.height=10}
hist <- list()
dataset_numerical<- select_if(dataset_clean, is.numeric)

i = 1
for (column in dataset_numerical) {
    dev.new()
    hist[[i]] <- ggplot(data = dataset_numerical, aes(x=column)) + 
    facet_grid(rows = vars(dataset_numerical$race_distance), scale = "free_y", space='free_y') +
    geom_histogram(binwidth=1, fill="#0c4c8a") +
    theme_minimal() +
    ggtitle(paste("Histograma -", column))
    i = i + 1
}
grid.arrange(hist[[1]],hist[[2]],hist[[3]],hist[[4]],hist[[5]], ncol=2)
...
dataset_clean$finish_time<-rescale(dataset_clean$finish_time)
dataset_clean$win_odds<-rescale(dataset_clean$win_odds)
```
...
mirem primer com estan distribuides les dades. i comprovem si compleixen amb la hipotesi de la distribució normal de les dades.
```{r} 
hist(dataset_clean$finish_time)
shapiro.test(dataset_clean$finish_time[1:5000]) #només precesa entre 3 i 5000 valors. mirar si falten valors fent aleatori la seleccio dels 5000
```



```{r} 
hist(dataset_clean$declared_horse_weight)
shapiro.test(dataset_clean$declared_horse_weight[1:5000]) #només precesa entre 3 i 5000 valors.
```
